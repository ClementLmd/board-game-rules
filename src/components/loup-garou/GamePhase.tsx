...
export function GamePhase({
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="sticky top-0 z-10 -mx-4 flex items-center justify-between gap-3 bg-warm-50/95 px-4 py-3 backdrop-blur sm:static sm:mx-0 sm:rounded-xl sm:border sm:border-gray-200 sm:bg-white sm:px-5 sm:py-4">
        <h2 className="text-xl font-bold text-gray-900 tabular-nums">
          Nuit {night}
        </h2>
        <div className="flex items-center gap-2">
          {canUndo && (
            <button
              type="button"
              onClick={onUndo}
              className="min-h-[44px] min-w-[44px] rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm active:bg-gray-50"
          ...
          <button
            type="button"
            onClick={onNextNight}
            className="min-h-[44px] rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm active:bg-primary-700"
          >
            Nuit suivante
          </button>
        </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <h3 className="border-b border-gray-100 px-4 py-3 text-sm font-semibold uppercase tracking-wide text-gray-500">
          Ordre des appels
        </h3>
        {nightSteps.length === 0 ? (
      ...
      <p className="text-center text-sm text-gray-600">
        En vie : <strong>{alive.length}</strong> — Éliminés : <strong>{dead.length}</strong>
      </p>
          ...
          {alive.map((p) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => onKill(p.id)}
                className="min-h-[44px] min-w-[44px] flex-shrink-0 rounded-xl bg-red-100 px-4 py-2.5 text-sm font-medium text-red-700 active:bg-red-200"
          ...
          <ul className="divide-y divide-gray-100">
            {dead.map((p) => (
              ...
              >
                ...
                {p.role && (
                  <span className="text-xs text-gray-500">{p.role.name}</span>
                )}
              </li>
            ))}
          </ul>
...
function NightCallRow({
  ...
  return (
    <li className="relative flex flex-wrap items-center gap-3 px-4 py-4 sm:flex-nowrap">
      <span
        className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 text-sm font-bold text-primary-700"
        aria-hidden
      >
        {num}
      </span>
      ...
      <div className="flex flex-shrink-0 items-center gap-2">
        <div className="relative">
          ...
          {showInfo && (
            <>
              ...
              />
              <div
                className="absolute right-0 top-full z-30 mt-2 w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-gray-200 bg-white p-4 shadow-lg"
                role="tooltip"
              >
        ...
        {isCupidonStep && showLoversButton && onOpenLoversForm && (
          <button
            ...
            className="min-h-[44px] flex-shrink-0 rounded-xl bg-primary-100 px-4 py-2.5 text-sm font-medium text-primary-700 active:bg-primary-200"
          >
...
function CupidonChoice({ players, onConfirm, onCancel }: CupidonChoiceProps) {
  ...
  return (
    <div className="rounded-2xl border border-primary-200 bg-primary-50/30 p-4 shadow-sm sm:p-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <select
            ...
            className="min-h-[44px] w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-base text-gray-900 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
      ...
      <div className="mt-6 flex gap-3">
        <button
          type="button"
          onClick={onCancel}
          className="min-h-[48px] flex-1 rounded-xl border border-gray-200 bg-white py-3 text-base font-medium text-gray-700 active:bg-gray-50"
        >
          Annuler
        </button>
        <button
          ...
          disabled={!canConfirm}
          className="min-h-[48px] flex-1 rounded-xl bg-primary-600 py-3 text-base font-medium text-white active:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
