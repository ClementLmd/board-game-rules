    ...
    <div>
      ...
      <div className="space-y-2">
        {notifications.map((notif) => {
          ...
          const link = data.competition_id ? `/competitions/${data.competition_id}/` : undefined;
          ...
          return (
            ...
            >
              <div className={`mt-0.5 w-2 h-2 rounded-full flex-shrink-0 ${notif.read ? 'bg-gray-200' : 'bg-violet-500'}`} />
              <div className="flex-1 min-w-0">
                <p className={`text-sm ${notif.read ? 'text-gray-600' : 'text-gray-900 font-medium'}`}>
                  {label}
                </p>
                {data.competition_name && (
                  <p className="text-xs text-gray-400 truncate">{data.competition_name}</p>
                )}
                {notif.type === 'result_validated' && data.validated_points && (
                  <p className="text-xs text-green-600 font-medium">+{data.validated_points} pts</p>
                )}
